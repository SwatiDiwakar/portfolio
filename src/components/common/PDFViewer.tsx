...
export default function PDFViewer({ pdfUrl, title, onClose }: PDFViewerProps) {
  ...
  // Set PDF.js worker in a useEffect so that it runs only on the client.
  useEffect(() => {
  ...
  }, [onClose, numPages]) // include dependencies as needed

  // IMPORTANT: Do not return early here if it prevents some hooks from running.
  // Instead, conditionally render the UI below.
  if (!mounted) {
